...
public class OAuth2ResourceServerApplicationITests {
	...
	void performPostWhenValidBearerTokenThenScopedRequestsAlsoWork() throws Exception {
		...
		// @formatter:off
		this.mvc.perform(post("/message").content("example message").with(bearerToken(this.messageWriteToken)))
				.andExpect(status().isOk())
	...
	void performPostWhenInsufficientlyScopedBearerTokenThenDeniesScopedMethodAccess() throws Exception {
		...
		// @formatter:off
		this.mvc.perform(post("/message").content("Example message").with(bearerToken(this.messageReadToken)))
				.andExpect(status().isForbidden())
				.andExpect(header().string(HttpHeaders.WWW_AUTHENTICATE,
						containsString("Bearer error=\"insufficient_scope\"")));
		// @formatter:on
