...
public class WechatTemplateServiceImpl implements WechatTemplateService {
	...
	public void templateReported(JSONObject reportInfo) {
		...
		/*
		 * 1、Kafka Producer是线程安全的,建议多线程复用,如果每个线程都创建,出现大量的上下文切换或争抢的情况,影响Kafka效率 2、Kafka
		 * Producer的key是一个很重要的内容： 2.1 我们可以根据Key完成Partition的负载均衡 2.2
		 * 合理的Key设计,可以让Flink、Spark Streaming之类的实时分析工具做更快速处理
		 * 
