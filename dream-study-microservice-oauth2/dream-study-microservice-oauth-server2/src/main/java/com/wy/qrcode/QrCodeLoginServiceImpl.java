...
public class QrCodeLoginServiceImpl implements IQrCodeLoginService {
	...
	public QrCodeLoginScanResponse scan(QrCodeLoginScanRequest loginScan) {
		...
		if (usernamePasswordAuthenticationToken.getPrincipal() instanceof OAuth2BaseUser) {
			OAuth2BaseUser basicUser = (OAuth2BaseUser) usernamePasswordAuthenticationToken.getPrincipal();
			// 生成临时票据
