...
public class OAuth2ResourcesServer extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.csrf(csrf -> csrf.disable())
				.formLogin(formLogin -> formLogin.loginProcessingUrl("/user/login").permitAll()
						.successHandler(clientLoginSuccessHandler).failureHandler(loginFailHandler))
				.authorizeRequests(authorizeRequest -> authorizeRequest
						.antMatchers(userProperties.getSecurity().getPermitSources()).permitAll().anyRequest()
						.authenticated());
	}
}