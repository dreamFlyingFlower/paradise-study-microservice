 ...
 * 
 * <pre>
 * GET(/oauth/authorize):授权登录页,登录成功重定向到授权页面.
 * eg:http://localhost:17127/oauth/authorize?client_id=test-client&response_type=code&scope=user&redirect_uri=https://www.baidu.com
 * 
 * 授权完成后重定向到认证成功的回调地址,上述地址为https://www.baidu.com,认证服务器会将code拼接在当前回调地址
 * 
 * POST(/oauth/token):从认证服务器获取令牌(access_token).
 * eg:http://localhost:17127/oauth/token?grant_type=authorization_code&client_id=test-client&code=code1111&redirect_uri=https://www.baidu.com
 * 
 ...
 * 
 * <pre>
 * POST(/oauth2/token):/oauth2/token?grant_type=client_credentials&scope=test1
 * 		请求头:
 * 			Authorization:Basic Base64编码({client_id}:{client_secret})
